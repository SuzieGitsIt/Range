# File:     Pycharm TkinterGUI_2023-04-10
# Version:  0.0.01
# Author:   Susan Haynes
# Comments/Notes:
...
#import PySimpleGUI as psg                       # Python popup window for sample entry fields
...
def get_samp ():
 ...
 #   global entry_s6, entry_d6, entry_s7, entry_d7, entry_s8, entry_d8, entry_s9, entry_d9, entry_s10, entry_d10
    ...
    win = tk.Tk()
    win.title("Sample Sizes and Dioptics")
    win.geometry('500x300')
    win.configure(background='white')  # Set background color
    win.option_add('*Font', 'Helvetica 12 bold')  # set the font and size for entire GUI
    win.option_add('*foreground', 'black')  # set the text color, hex works too '#FFFFFF'
    win.option_add('*background', 'white')  # set the background to white

    entry_s1 = tk.Entry(win, width=10)
    entry_s1.focus_set()  # Places cursor in the first entry box.
    entry_s1.place(x=120, y=25)
    entry_s2 = tk.Entry(win, width=10)
    entry_s2.place(x=120, y=50)
    entry_s3 = tk.Entry(win, width=10)\
    entry_.place(x=120, y=75)
    entry_s4 = tk.Entry(win, width=10).place(x=120, y=100)
    ...
    def sav_all():
        ...
        s3 = entry_s3.get()[:2]
        s4 = entry_s4.get()[:2]
        total = [entry_s1, entry_s2, entry_s3, entry_s4, entry_s5, entry_s6, entry_s7, entry_s8, entry_s9, entry_s10]
        Sum = sum(total)
        print("Total # of samples: ", Sum)
        print("Total # of sets: ") # Will need to get the number of sets somehow... maybe check for blank entries and sum the full ones?
        print("entry_s1: ", entry_s1)
        print("s1: ", s1)
        ws.cell(GUI, column=3, row=new_line).value = entry_s1  # Why does this not always get printed to the workbook???
        ws.cell(win, column=3, row=new_line).value = entry_s2  # Why does this not always get printed to the workbook???
        ws.cell(GUI, column=3, row=new_line).value = entry_s3  # Why does this not always get printed to the workbook???
        ws.cell(win, column=3, row=new_line).value = entry_s4  # Why does this not always get printed to the workbook???

    def win_cls():
        win.destroy()

    btn_ok = tk.Button(win, text='Save All', width=5, command=sav_all)
    btn_ok.place(x=400, y=250)
    btn_cls = tk.Button(win, text='Close', width=5, command=win_cls)
    btn_cls.place(x=450, y=250)
...
def fun_samp(entry_samp):
    ...
    print("Total # of samples: ", Sum)
    print("Total # of sets: ")                                           # Will need to get the number of sets somehow... maybe check for blank entries and sum the full ones?
    print("entry_s1: ", entry_s1.get()[:2])                              # Print can be removed after developed.
    s.cell(column=3, row=new_line, value=entry_s1.get()[:2])
    s.cell(column=3, row=new_line, value=entry_s2.get()[:2])
    s.cell(column=3, row=new_line, value=entry_s3.get()[:2])
    s.cell(column=3, row=new_line, value=entry_s4.get()[:2])
    total = [entry_s1.get(),entry_s2.get(),entry_s3.get(),entry_s4.get(),entry_s5,entry_s6,entry_s7,entry_s8,entry_s9,entry_s10]
    Sum = sum(total)
